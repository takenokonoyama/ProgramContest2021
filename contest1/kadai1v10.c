...
void ThreeTwoOpt_array_n2_with_SA(struct point p[MAX_N], int N, int Tour[MAX_N]) {
  int a, b, c, d, e, f;
  int I;
  int fImproved = 0;
  int NewTour[MAX_N];
  ...
  while(1) {
    for (int i = 0; i < N-4; i++) {
      for (int j = i+1; j < N-2; j++) {
        ...
        D[7] = dist(p[a],p[c]) + dist(p[b],p[d]) + dist(p[e],p[f]);
        for (int m = 0; m <= 7; m++) {
          if (D[m] < D[min]) min = m;    // 7通りのつなぎ替えを比較
        }
        if (min == 0) {
          min_sa = rand() % 4 + 1;
          //printf("min_sa %d\n", min_sa);
...
int main(int argc, char *argv[]){
  ...
  nn_array(p,n,tour);
	...
	// 3-2opt焼きなまし法による改善
	ThreeTwoOpt_array_n2_with_SA(p,n,tour);
