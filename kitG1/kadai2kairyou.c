...
int TwoOpt_list(struct point p[MAX_N], int n, struct list* visited, int m, int prec[MAX_N]){
  int a,b,c,d;
  int tmp;
  int chk=0;
  int count;
...
void loop(struct point p[MAX_N], int n, int tour[MAX_N], int pm, int prec[MAX_N]){
  ...
  printf("%5.1lf\n",tour_length_array(p,n,newtour)); // 巡回路tourの長さ
	...
	initialize(vis);
	for(int i=0; i<n; i++) {
	  insertBefore(vis->tail, newtour[i]);
	}
	...
	while(chkk == 1) {
		...
		for(int k = 8; k >= 1; k--){
			if(chk_array[k] == 1){
				chk_array[k] = OrOptk_list(k, p, n, vis, pm, prec);
				if(chk_array[k] == 2) {
					chkk = 2;
					break;
				}
...
int main(int argc, char *argv[]) {
  ...
  while(chk2){
  	// 2-optによる改善
		chk2 = TwoOpt_list(p,n,visited,m,prec);
	}
	
	printf("2opt done\n");
	write_tour_data_array("tour1.dat",n,tour);
	printf("%5.1lf\n",tour_length_list(p,n,tour));
	/*
	// chk配列を初期化
	for(int i = 0; i < 10; i++) chk_array[i] = 1;
	chkk = 1;
	...
	//OrOpt1_prec_array(p, n, tour, m, prec);
	//printf("orOpt1_prec done\n");
  //printf("%5.1lf\n",tour_length_array(p,n,tour)); // 巡回路tourの長さ
