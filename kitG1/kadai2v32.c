...
void ei(struct point p[MAX_N], int n, int tour[MAX_N],int m, int prec[MAX_N]){
  FILE *fp;
  int i,j,a,b,c,r;
  int visited[MAX_N]; // 都市iが訪問済みならば1そうでなければ0
  double d[MAX_N]; // 未訪問点 r から現在の部分巡回路までの最短距離を d[r] に保存
...
void OrOpt1_with_sa_array(struct point p[MAX_N], int n, int tour[MAX_N], int pm, int prec[MAX_N], int nowtemp) {
  int a,b,c,d,e;
  int i,j,k,l,m,h;
  int tmp, chk=0;
  double prob;
...
double loop_array(struct point p[MAX_N], int n, int tour[MAX_N], int pm, int prec[MAX_N], int nic, int exflag, double best){
  ...
  OrOpt1_with_sa_array(p, n, newtour, pm, prec, 100+nic*10);
