...
double Probability(float exchange_cost, float t){
  return exp(-exchange_cost/t);
}
double Temperature(int now_time){
  return 1000-pow(1000,(float)(now_time)/TIME_LIMIT);
}
		...
		for(k=i+2;k<=n-1;k++){
  		...
  		printf("%f", temp);
			if(excost > 0) {
				...
				double prob=Probability(excost, (float)now_time);
				printf("%f\n", prob);
				if(0.5 <= prob){
					g=j;
					h=k;
