...
void nn_array_starts(struct point p[MAX_N], int n, int tour[MAX_N]){
  int i,j,r,nearest=-1;
  int a;
  int s;
  ...
  double min;
	...
	while(count <= count_max){
		...
		s = rand() % n;     // startする都市
		tour[0]= s;         // 最初に訪問する都市は s 
  	visited[s]=1;    		 // 都市sは訪問済み
		for(i=0;i<n-1;i++) {
		  a = tour[i];
		...
		sum = tour_length_array(p, n, tour);
		printf("%f\n", sum);
		if(maxsum < sum) {
			minsum = sum;
			for(int k = 0; k < n; k++) tour_min[k] = tour[k];
		}
		count++;
	}
	for(int k = 0; k < n; k++) tour[k] = tour_min[k];
}
...
void TwoOpt_array2(struct point p[MAX_N], int n, int tour[MAX_N]){
  int a,b,c,d;
  int i,j,k,l,g,h;
  int chk=0;
...
int main(int argc, char *argv[]) {
  ...
  dnn_array(p,n,tour);
  // 巡回路をテキストファイルとして出力
  write_tour_data_array("tourdnn.dat",n,tour);
  // 巡回路長を画面に出力
