  ...
  double min;
	...
	if(p[s1].x != p[s2].x){
		for(i = 0; i < n; i++){
			...
			if(p[i].y >= (p[s2].y - p[s1].y)/(p[s2].x-p[s1].x)*(p[i].x - p[s1].x) + p[s1].y){
				group1[gn1] = i;
				gn1++;
			}
			else{
	...
	for(i=0;i<gn1;i++) {
	  ...
	  for(r=0;r<gn1;r++) {
	  	b = group1[r];
	    if(!visited1[b] && dist(p[a],p[b])<min){ 
	      nearest=b; //都市tour[i]から暫定的に最短距離にある未訪問都市をnearestとする
	      min = dist(p[a],p[b]); // その暫定最短距離をminとする
	    }
	  }
	  tour[i+1]=nearest; // i+1 番目に訪問する都市を nearest にして, 
	  visited[nearest]=1;// nearest を訪問済みとする. 
	...
	tour[gn1+1] = s2;
	visited[s2] = 1;
	//s2スタート地点としてgroup2内の点に対してNN
	for(i=gn1+1; i < gn1+1+gn2;i++) {
	  a = tour[i];
	  //最後に訪問した都市 a == tour[i]から最短距離にある未訪問都市nearestを見つける
	  min = INF;         
	  for(r=0;r<gn2;r++) {
	  	b = group2[r];
	    if(!visited2[b] && dist(p[a],p[b])<min){ 
	      nearest=b; //都市tour[i]から暫定的に最短距離にある未訪問都市をnearestとする
	      min = dist(p[a],p[b]); // その暫定最短距離をminとする
	    }
	  }
