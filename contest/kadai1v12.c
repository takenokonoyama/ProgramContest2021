...
// nearest neigbor except prec
void nn_array_expr(struct point p[MAX_N], int n, int tour[MAX_N], int m, int prec[MAX_N]){
  int i,j,r,nearest=-1;
  int start;
  int visited[MAX_N]; // 
  double min;
  ...
  for(r=0;r<n; r++) {
  	for(p = 0; p < m; p++){
  		if(r == prec[p]) visited[r]=-1; // 最初は全都市は未訪問
  		else visited[r] = 0;
  	}
  }
...
// 単純反復(2opt焼きなましから)
void loop (struct point p[MAX_N], int n, int tour[MAX_N]){
	int newtour[MAX_N];
	int chk2 = 1;
	int chk3 = 1;
	int chk32 = 1;
  ...
  struct list* vis = &visited;
	...
	chk32 = 1;
	while(chk32) {
		chk32 = ThreeTwoOpt_array_n2(p, n, tour);
	}
	// 3Opt(高速化)による改善
	chk3 = 1;
	while(chk3) {
		chk3 = ThreeOpt_array_n2(p, n, tour);
	}
