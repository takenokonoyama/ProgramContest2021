...
void shuffle(int array[MAX_N], int size) {
...
void reverse(int array[MAX_N], int size) {
  int i = 0;
  int j = size;
  while(i < j) {
    int tmp = array[i];
...
void ss(int n, int tour[MAX_N], int m, int prec[MAX_N]) {
  int chk1 = 0;
  int i = 0;
  while(chk1 != 2) {
    for(int j = 0; j < m; j++) {
...
void loop_array(struct point p[MAX_N], int n, int tour[MAX_N], int pm, int prec[MAX_N]){
	int newtour[MAX_N];
	int chk1 = 0;
	int chk2 = 1;
	int chk3 = 1;
  ...
  */
  ...
  //printf("count:%d\n", count);
  //reverse(newtour, count-2);
	//shuffle(newtour, count-3);
	//ss(n, newtour, pm, prec);
	TwoOpt_with_SA_array(p, n, newtour, pm, prec);
  printf("bad done \n");
  printf("%5.1lf\n",tour_length_array(p,n,newtour)); // 巡回路tourの長さ
  ...
  while(chk2or){
  	// 2-optによる改善
		chk2or = CombinationOpt(p,n,&visited,pm,prec);
	}
	printf("%5.1lf\n",tour_length_list(p,n,&visited));
...
double loop_list(struct point p[MAX_N], int n, struct list* visited, int pm, int prec[MAX_N], double best){
	int tour[MAX_N];
	int chk2 = 1;
	int chk3 = 1;
	int chk32 = 1;
  ...
  while(chk2){
  	// 2-optによる改善
		chk2 = TwoOpt_array(p,n,tour,pm,prec);
	}
	//printf("2opt done\n");
	//printf("%5.1lf\n",tour_length_array(p,n,tour));
